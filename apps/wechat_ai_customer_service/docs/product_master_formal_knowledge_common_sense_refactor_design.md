...
6. 所有客户可见回复由 `customer_service_brain` 发出并最终决定，结构化层只负责提供权威证据、候选、风险和审稿意见；Brain 不可用或不可采纳时阻断发送，不发送本地 fallback。
7. 发现 Brain 输出不合适时，默认返修给 Brain 重新思考；不得通过新增局部结构化话术、车型分支或关键词模板来替代 Brain。
...
以下优先级只定义“哪些证据可以授权事实”，不定义“由谁最终组织回答”。最终理解客户意图、选择回复策略、回应质疑、陪聊或转人工的主体必须是 `customer_service_brain`。

结构化层、RAG、AI经验池、LLM常识层、质量门、guard、最终润色只向 Brain 提供证据、候选、边界、审稿反馈或轻量表达建议。发现问题时，应将意见交给 Brain 返修，不允许本地模板直接接管客户可见回答。
...
- AI经验池、历史聊天、style memory、LLM常识不能授权商品事实，只能帮助 Brain 理解问法和表达取舍。
...
- AI经验池和常识层只能辅助 Brain 判断客户在问什么、如何解释更自然，不能授权流程承诺。
...
- 常识层的结论应作为 Brain 的推理材料，而不是本地结构化模板的直接输出。
...
- RAG只改表达，不改事实。
- 最终回复必须由 Brain 主控，并经过事实守卫与弱润色检查。
