"""Scoped operator guard launcher for the add_friend RPA flow.

This module deliberately reuses ``scripts/run_rpa_operator_guard.py`` for the
actual keyboard/mouse hook and floating indicator. It only owns the add_friend
runtime paths, startup verification, pause/stop checkpointing, and cleanup.
"""
...
def add_friend_operator_guard_settings() -> dict[str, Any]:
    hotkey = str(
        os.getenv("WECHAT_ADD_FRIEND_OPERATOR_GUARD_CONTROL_HOTKEY")
        or os.getenv("WECHAT_RPA_OPERATOR_GUARD_CONTROL_HOTKEY")
        or "f8"
    ).strip().lower()
    ...
    return {
        "enabled": first_env_bool(
            ("WECHAT_ADD_FRIEND_OPERATOR_GUARD_ENABLED", "WECHAT_RPA_OPERATOR_GUARD_ENABLED"),
            default=os.name == "nt",
        ),
        ...
        "esc_double_press_window_ms": bounded_int(
            os.getenv("WECHAT_ADD_FRIEND_OPERATOR_GUARD_ESC_DOUBLE_WINDOW_MS")
            or os.getenv("WECHAT_RPA_OPERATOR_GUARD_ESC_DOUBLE_WINDOW_MS"),
            default=420,
            minimum=180,
            maximum=1200,
        ),
        "pause_poll_interval_ms": bounded_int(
            os.getenv("WECHAT_ADD_FRIEND_OPERATOR_GUARD_PAUSE_POLL_INTERVAL_MS")
            or os.getenv("WECHAT_RPA_OPERATOR_GUARD_PAUSE_POLL_INTERVAL_MS"),
            default=550,
            minimum=120,
            maximum=3000,
        ),
        "bootstrap_timeout_seconds": bounded_float(
            os.getenv("WECHAT_ADD_FRIEND_OPERATOR_GUARD_BOOTSTRAP_TIMEOUT_SECONDS")
            or os.getenv("WECHAT_RPA_OPERATOR_GUARD_BOOTSTRAP_TIMEOUT_SECONDS"),
            default=15.0,
            minimum=3.0,
            maximum=60.0,
        ),
        "pause_max_seconds": bounded_float(
            os.getenv("WECHAT_ADD_FRIEND_OPERATOR_GUARD_PAUSE_MAX_SECONDS"),
            default=600.0,
            minimum=5.0,
            maximum=3600.0,
        ),
...
def add_friend_operator_guard_dir(tenant_id: str | None = None) -> Path:
    return tenant_runtime_root(tenant_id) / "add_friend" / "operator_guard"


def add_friend_operator_guard_paths(tenant_id: str | None = None) -> dict[str, Path]:
    root = add_friend_operator_guard_dir(tenant_id)
    return {
        "root": root,
...
def verify_add_friend_operator_guard(
    *,
    pid: int,
    state_path: Path,
...
def start_add_friend_operator_guard(*, route: str = "", artifact_dir: str | None = None) -> dict[str, Any]:
    ...
    settings = add_friend_operator_guard_settings()
    tenant_id = active_tenant_id()
    paths = add_friend_operator_guard_paths(tenant_id)
    base = {
        "ok": True,
    ...
    if existing_pid and pid_alive(existing_pid):
        try:
            sync_operator_mode(paths["control_path"], tenant_id=tenant_id, mode="stopped", message="replace_stale_add_friend_guard")
            started = time.monotonic()
    ...
    write_json(
        paths["status_path"],
        {
            "ok": True,
            "state": "thinking",
            "message": "加好友 RPA 正在运行，悬浮球键鼠守护已接管。",
            "tenant_id": tenant_id,
        },
    )
    command = [
    ...
    verify = verify_add_friend_operator_guard(
        pid=int(proc.pid),
    ...
    if result["ok"]:
        ...
        return result
    stop_add_friend_operator_guard(result, reason="operator_guard_verify_failed")
    _ACTIVE_GUARD = None
    return result
...
def add_friend_operator_guard_checkpoint(*, reason: str = "") -> dict[str, Any]:
    ...
    while True:
        ...
        if mode == "stopped":
            raise RuntimeError(f"add_friend_operator_guard_stopped:{reason}")
        if mode != "paused":
            return {
                "ok": True,
        ...
        if time.monotonic() - started >= pause_max_seconds:
            raise RuntimeError(f"add_friend_operator_guard_pause_timeout:{reason}")
        time.sleep(0.2)
