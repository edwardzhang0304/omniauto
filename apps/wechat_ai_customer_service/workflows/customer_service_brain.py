...
def apply_universal_brain_runtime_settings(
    ...
    *,
    ...
    compact["quality_repair_max_tokens"] = DEFAULT_REPAIR_MAX_TOKENS
...
def build_low_authority_fast_evidence_pack(
    *,
    ...
    if chejin_context:
        ...
        history = trusted_recent_multimodal_messages(target_state)
        history_text = trusted_recent_multimodal_history_text(target_state)
...
def maybe_retry_brain_repair_after_empty_plan(
    *,
    ...
    if retry.get("ok") and not normalize_reply_segments(
        ...
        max_segments=int(settings.get("max_reply_segments") or 3), preserve_all=settings.get("reply_sequence_version") == 1,
    ):
        retry["ok"] = False
    ...
    prompt_input = {
        ...
        "conversation": {
            ...
            "history_text": (
                ...
                if chejin_authoritative_history
                else _compact_recent_history_text(
                    str(conversation.get("history_text") or ""),
...
def run_brain_repair_llm(
    *,
    ...
    if not isinstance(parsed, dict):
        ...
        response["raw_response_text"] = raw_text[:1000]
        return response
    if not normalize_reply_segments(parsed.get("reply_segments"), max_segments=int(settings.get("max_reply_segments") or 3), preserve_all=settings.get("reply_sequence_version") == 1):
        if settings.get("_single_brain_runtime_cleanup"):
            response["ok"] = False
...
def compact_product_item_for_brain_prompt(item: dict[str, Any], *, max_text_chars: int) -> dict[str, Any]:
    ...
    if specs:
        compact["specs"] = specs if item.get("source_type") == "chejin_backend" else compact_product_specs_for_brain_prompt(specs, max_text_chars=max_text_chars)
