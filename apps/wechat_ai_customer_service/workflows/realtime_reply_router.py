...
def decide_realtime_reply_route(
    *,
    ...
    def _common_sense_advisor_route(reason: str) -> dict[str, Any]:
        payload = {
            **route,
            "level": "L1",
            "reason": reason,
        ...
        if common_sense_advisor_allowed:
            payload["advisor_mode"] = "clear_common_sense_recommendation"
            payload["advisor_goal"] = "give_a_clear_recommendation_when_safe"
        return payload
...
def is_existing_vehicle_option_compare(text: str) -> bool:
    ...
    compare_markers = ("哪个", "哪台", "哪款", "更适合", "更好", "怎么选", "好停", "好开", "停车")
...
def has_named_compare_options(text: str) -> bool:
    ...
    if not contains_any(clean, ("和", "跟", "与", "、", "还是", "或者", "对比", "比较")):
        return False
    option_markers = ("哪个", "哪台", "哪款", "哪一类", "先看", "更适合", "更好", "怎么选", "建议")
...
def meaningful_customer_compare_options(text: str) -> list[str]:
    ...
    for option in extract_customer_compare_options(text):
        ...
        if contains_any(normalized, ("预算", "车况", "油耗", "家用", "通勤", "省心", "透明", "别太高")) and not contains_catalog_product_term(normalized):
            continue
        if contains_any(normalized, ("跑客户", "放样品", "二胎", "家庭", "接娃", "买菜", "老人孩子")) and not contains_catalog_product_term(normalized):
            continue
...
def extract_customer_compare_options(text: str) -> list[str]:
    ...
    if "里面" in fragment and "在" in fragment:
        ...
        fragment = fragment.rsplit("先看", 1)[-1]
    else:
        for marker in ("哪个", "哪台", "哪款", "哪一类", "怎么选", "更适合", "更好", "建议"):
            if marker in fragment:
                fragment = fragment.split(marker, 1)[0]
                break
    ...
    ignored = {"", "哪个", "哪台", "哪款", "哪一类", "选", "里面选", "你建议我", "您建议我"}
    options: list[str] = []
...
def build_vehicle_compare_reply(query: str, *, recent_reply_texts: list[str] | None = None) -> tuple[str, int]:
    ...
    family_mpv_context = contains_any(context, ("家用", "孩子", "老人", "二胎", "一家", "全家", "七座", "mpv", "mpv或大七座")) and contains_any(
        context,
        ("gl8", "奥德赛", "mpv", "七座"),
    )
    business_negative_context = contains_any(context, ("不想商务", "商务味太重", "别太商务", "不要商务", "商务感太重"))
    ...
    economy_transparency_context = contains_any(context, ("车况透明", "车况清楚", "油耗别高", "省油", "油耗", "后期成本", "维护成本"))
    ...
    if spouse_or_new_driver_context:
        ...
        variants = option_context_variants(
            option_label,
            [
                "按新手/好停这个前提，{options}里我会先看车身更小、视野更好、倒车影像/雷达更齐的那台；尺寸大的先放备选。",
                "{options}可以对比，但这个场景建议先排好开好停、省心成本低的方案，空间或动力更强的做备选。",
        ...
        variants = option_context_variants(
            option_label,
            [
                "{options}里，家用我会先偏奥德赛这类，坐姿和油耗更适合家庭；GL8空间和气场更强，但商务感会重一点。到店再看第三排、行李空间和检测报告。",
        ...
        if preferred:
            ...
            backup_text = "、".join(label for label in backup_labels if label)
            variants = [
                f"公司用兼顾接客户，我会先看{preferred_label}：接待观感更稳，后备厢和通过性也够用；{backup_text}性价比高，可以当备选。",
                f"这两台里{preferred_label}优先，更适合公司接待和拉物料一起用；{backup_text}预算优势明显，但接待观感略弱一点。",
                f"按活动物料加接客户这个场景，我会把{preferred_label}排前面，{backup_text}放备选。到店再重点核后备厢、二排放倒和检测报告。",
            ]
            return choose_natural_reply_variant(variants, key_text=query + recent, recent_reply_texts=recent_reply_texts)
        variants = option_context_variants(
            option_label,
...
def option_context_variants(option_label: str, with_options: list[str], fallback: list[str]) -> list[str]:
    if not option_label:
        return fallback
    return [template.format(options=option_label) for template in with_options]
    ...
    if contains_any(normalized, ("suv", "停车", "老婆", "爱人", "偶尔也会开", "市区")):
        if spouse_or_new_driver_context:
            ...
            variants = [
                ...
                "公司用车不能只看空间大。市区跑得多的话，车身尺寸、油耗、后备厢实用性和接待观感都要平衡；SUV方向可以看，但必须结合商品库里的实车车况筛。",
...
def build_recommendation_reply(
    ...
    *,
    ...
    if summary:
        variants = [
            f"按{summary}，我会先看{product_text}。价格更贴近，后面重点核检测报告和实车车况。",
            f"{summary}这个区间，{product_text}更值得先看。车况确认后再谈贷款/置换。",
            f"先排{product_text}，更贴近{summary}。到店主要比检测报告、公里数和实际车况。",
            f"按{summary}，先缩到{product_text}。车况没问题，再继续谈贷款/置换。",
        ]
    else:
        variants = [
            f"我会先看{product_text}。{candidate_ref}方向比较清楚，后面主要比检测报告、公里数和实车状态。",
            f"先从{product_text}里挑。二手车别只看车名，检测报告和实车车况更关键。",
            f"{product_text}可以先放前面。到店再把车况、公里数和价格一起核。",
            f"先给您缩到{product_text}。如果车况没问题，再继续谈贷款或置换。",
        ]
